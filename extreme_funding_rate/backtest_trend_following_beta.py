"""
Trend Following Strategy with BETA Hedge.
- SHORT the most extreme negative funding coin (follow the trend)
- LONG BTC with beta-adjusted position size (30-day rolling beta)
...
class TrendFollowingBetaHedgedBacktest:
    """Backtest engine for trend following with beta-adjusted BTC hedge."""
    ...
    def simulate_trade(self, coin: str, entry_time: pd.Timestamp, exit_time: pd.Timestamp,
        ...
        """
        Simulate a beta-hedged TREND FOLLOWING trade.
        
        SHORT coin + LONG beta*BTC (opposite of mean reversion)
        """
        # Get prices
        ...
        # SHORT coin position (profit if price falls)
        coin_position = (coin_capital - total_entry_cost/2) / coin_entry
        coin_pnl = coin_position * (coin_entry - coin_exit)  # Profit if coin falls
        
        # LONG BTC hedge (beta-adjusted) - profit if BTC rises
        btc_position = (btc_capital - total_entry_cost/2) / btc_entry
        btc_exit_value = btc_position * btc_exit
        btc_pnl = btc_exit_value - btc_capital
        
        # Total exit value and costs
        total_exit_value = (coin_capital + coin_pnl) + (btc_capital + btc_pnl)
        ...
        # For SHORT coin + LONG BTC: hedged = -coin_return + beta * btc_return
    ...
    def run_backtest(self, df: pd.DataFrame) -> pd.DataFrame:
        """Run trend following + beta hedge backtest."""
        print(f"\n🚀 Running TREND FOLLOWING + BETA HEDGE backtest...")
    ...
    def save_results(self, df_trades: pd.DataFrame, metrics: Dict, prefix='backtest_tf_beta'):
        """Save results to CSV."""
...
def main():
    print("=" * 70)
    print("TREND FOLLOWING + BETA HEDGE BACKTEST")
    print("(SHORT extreme negative funding + LONG beta*BTC)")
    print("=" * 70)
    ...
    # Initialize backtest
    try:
        backtest = TrendFollowingBetaHedgedBacktest(
    ...
    # Display results
    print("\n" + "=" * 70)
    print("FINAL RESULTS - TREND FOLLOWING + BETA HEDGE")
    print("=" * 70)
