...
parser = argparse.ArgumentParser(description="Run all strategies on specified datasets")
parser.add_argument("--funding-file", default="funding_history.csv", help="Funding CSV file")
parser.add_argument("--price-file", default="price_cache_with_beta_history.csv", help="Price CSV file")
parser.add_argument("--output-prefix", default=None, help="Optional output prefix for result files")
args = parser.parse_args()

# Load data
print("Loading data...")
price_cache = pd.read_csv(args.price_file)
...
funding = pd.read_csv(args.funding_file)
...
results_out = f"{args.output_prefix}_strategy_comparison_all8.csv" if args.output_prefix else "strategy_comparison_all8.csv"
...
def plot_equity_curves(results):
    ...
    plt.tight_layout()
    out_name = f"{args.output_prefix}_equity_curves.png" if args.output_prefix else "equity_curves.png"
...
def plot_performance_comparison(results):
    ...
    plt.tight_layout()
    out_name = f"{args.output_prefix}_performance_comparison.png" if args.output_prefix else "performance_comparison.png"
    plt.savefig(out_name, dpi=150, bbox_inches='tight')
    print(f"📊 Saved: {out_name}")
    plt.close()
...
def plot_drawdown_curves(results):
    ...
    plt.tight_layout()
    out_name = f"{args.output_prefix}_drawdown_curves.png" if args.output_prefix else "drawdown_curves.png"
    plt.savefig(out_name, dpi=150, bbox_inches='tight')
    print(f"📊 Saved: {out_name}")
    plt.close()
...
def plot_cumulative_pnl_distribution(results):
    ...
    plt.tight_layout()
    out_name = f"{args.output_prefix}_cumulative_pnl.png" if args.output_prefix else "cumulative_pnl.png"
    plt.savefig(out_name, dpi=150, bbox_inches='tight')
    print(f"📊 Saved: {out_name}")
    plt.close()
...
def plot_summary_dashboard(results):
    ...
    plt.suptitle('Extreme Funding Rate Strategy Backtest - Summary', fontsize=14, fontweight='bold', y=0.98)
    
    out_name = f"{args.output_prefix}_backtest_dashboard.png" if args.output_prefix else "backtest_dashboard.png"
    plt.savefig(out_name, dpi=150, bbox_inches='tight')
    print(f"📊 Saved: {out_name}")
    plt.close()
...
if args.output_prefix:
    print(f"   - {args.output_prefix}_equity_curves.png")
    print(f"   - {args.output_prefix}_performance_comparison.png")
    print(f"   - {args.output_prefix}_drawdown_curves.png")
    print(f"   - {args.output_prefix}_cumulative_pnl.png")
    print(f"   - {args.output_prefix}_backtest_dashboard.png")
else:
