...
def main():
    ...
    sig_paths = {tf: base / f"xyz:mstr_{tf}_rolling_beta_signals.csv" for tf in TIMEFRAMES}
    met_paths = {tf: base / f"xyz:mstr_{tf}_rolling_beta_metrics.json" for tf in TIMEFRAMES}
    ...
    axes[0].set_title("xyz:MSTR vs BTC Net Equity: 1H vs 4H vs 1D")
    ...
    fig.tight_layout()
    out_png = base / "xyz_mstr_1d_vs_4h_fee_comparison.png"
    fig.savefig(out_png, dpi=160)
    plt.close(fig)

    rec = recommend(metrics)
    out_md = base / "xyz_mstr_1d_vs_4h_evaluation.md"
    lines = [
